...
1、程序通过 `cargo run` 编译并启动后，Mac 机上通过 `lsof -i:8899` 可以看到 `TCP localhost:8866 (LISTEN)` 表示监听启动成功。
2、客户端通过 telnet 进行测试，测试命令 : `telnet localhost 8899` 输入 ASCII 字符返回响应字符测试通过。
3、为了可以连接多个客户端，需要使用 thread 类库对链接使用线程进行处理。
4、输入`bye`程序退出，并提示 `Bye bye and see you soon.`
转自: https://www.jianshu.com/p/db74f18bfdc3
 */
...
fn main() {
    // 定义一个请求地址 IP:端口 的形式
    let addr = "127.0.0.1:8899".to_string();
